  ...
  <div class="col q-px-xl q-py-lg">
    ...
    <div class="row q-pa-md q-mt-md shadow-1" style="background-color: white;">
      <div class="col q-pa-md div-padre">
        ...
        <p class="text-h5 text-center">{{ this.firstGraph.name }}</p>
        <graphics :graph="this.firstGraph"></graphics>
      </div>
      <div class="col q-pa-md div-padre text-center">
        <div class="div-hijo">
        </div>
...
export default {
  ...
  data () {
    return {
      ...
      firstGraph: {
        id: 'firstGraph',
        name: 'Rellena los filtros de la parte de arriba',
        headers: [],
        results: [],
        label: 'MXN Peso'
      },
      secondGraph: {
        id: 'secondGraph',
        name: 'para visualizar las graficas',
        headers: [],
        results: [],
        label: 'Unidades'
      }
  ...
  methods: {
      ...
      for (var i = 0; i < json.length; i++) {
        for (var j = 0; j < json[i].length; j++) {
          ...
          for (var k = 0; k < json[i][j].length; k++) {
            ...
            unidades[k] += json[i][j][k][0]
            if (isNaN(suma[k]) || suma[k] === 0) {
              suma[k] = 0
            }

            if (isNaN(unidades[k]) || unidades[k] === 0) {
              unidades[k] = 0
            }
          }
        }
    ...
    createCSV: function () {
      ...
      const fields = ['2019/08/26.Guadalupe.Indio.unidades', '2019/08/26.Guadalupe.Indio.precio']
      const myCars = fakeData
